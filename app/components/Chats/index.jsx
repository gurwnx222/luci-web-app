...
export default function ChatPage() {
  ...
  return (
    <div className="h-full flex flex-col bg-[#262628] rounded-4xl overflow-hidden">
      ...
      <div className="bg-[#EED4CF] rounded-3xl  px-8 py-6">
        <div className="flex  gap-6 overflow-x-auto no-scrollbar">
          {users.map((user) => (
            <div
              key={user.id}
              onClick={() => setActiveUser(user)}
              className={`min-w-[90px] w-22  cursor-pointer flex flex-col items-center gap-2 p-3 rounded-2xl ${
      ...
      <div className="flex-1 bg-[#EED4CF] mt-2 rounded-t-3xl  overflow-y-auto px-16 py-8 space-y-10 chat-scroll">
      ...
      {/* 🔹 INPUT BAR */}
      <div className="bg-[#EED4CF]   px-10 py-6">
        <div className="flex items-center gap-4 bg-[#fff5f1] px-6 py-4 rounded-2xl">
          <button
            type="button"
          ...
          <input
            ...
            className="flex-1 text-[#262628] bg-transparent outline-none text-sm"
          />
          <button onClick={sendMessage} className="text-[#D96073] text-lg">
            ➤
          </button>
        </div>
      </div>
